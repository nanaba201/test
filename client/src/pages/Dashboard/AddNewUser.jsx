...
//                     required
//                   />
//                   <div style={{ width: "82%", borderRadius: 6 }}>
//                     <Dropdown
//                       options={options}
//                       value={userData.role}
//                       onChange={(selected) =>
...
import React, { useState, useEffect } from "react";
import { Outlet } from "react-router-dom";
import {API_URL, register} from "../../services/api_service";
import Sidebar from "../../components/Sidebar";
import Dropdown from "react-dropdown";
import "react-dropdown/style.css";
import { ToastContainer, toast } from "react-toastify";
...
function AddNewUser() {
  // Mapping for display and API values
  const roleMapping = {
    Teacher: "teacher",
    Student: "student",
  };

  const options = Object.keys(roleMapping); // ["Teacher", "Student"]
  const [selectedRole, setSelectedRole] = useState("Student");
  const [isOpen, setIsOpen] = useState(false);
  const [users, setUsers] = useState([]);
  const [userData, setUserData] = useState({
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const apiRole = roleMapping[selectedRole]; // Convert to lowercase
        const response = await fetch(
          `${API_URL}/fetch_users?role=${apiRole}`
        );
        const result = await response.json();
        if (result.success) {
          setUsers(result.users);
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    try {
      const apiRole = roleMapping[userData.role]; // Convert to lowercase
      const response = await register(
          {
            ...userData,
            role: apiRole,
      ...
      // Re-fetch users for the selected role
      const updatedResponse = await fetch(
        `${API_URL}/fetch_users?role=${roleMapping[selectedRole]}`
      );
      const updatedResult = await updatedResponse.json();
      if (updatedResult.success) {
        setUsers(updatedResult.users);
